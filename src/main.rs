...
impl App {
    ...
    #[allow(unused_mut)]
    fn control_panel_window_main(&self, args: &Vec<String>) {
        /*
         * Web UI subprocess
         *
        ...
        let http_client = HTTPClient::new(vec!["status", "network", "peer"], 100, 5);
        ...
        let ui_client = http_client.clone();
            ...
            .with_rpc_handler(
                move |window: &wry::application::window::Window,
                    ...
                    match req.method.as_str() {
                        ...
                        "delete" => {
                            arg.as_str().map(|path| {
                                ui_client.client.lock().enqueue_delete(path.to_string())
                            });
                            None
                        }
                        "remember_network" => {
                            let _ = arg.as_array().map(|p| {
                                if p.len() == 3 {
                                    ...
                                    let settings =
                                        p.get(2).map_or("", |s| s.as_str().unwrap_or(""));
                                    ui_client.client.lock().remember_network(
                                        nwid.to_string(),
                                        name.to_string(),
                                        settings.to_string(),
                                    );
                        ...
                        "forget_network" => {
                            arg.as_str().map(|path| {
                                ui_client.client.lock().forget_network(&path.to_string())
                            });
                            None
                        }
                        "copy_to_clipboard" => {
                        ...
                        "poll" => {
                                ...
                                .swap(false, std::sync::atomic::Ordering::Relaxed)
                            {
                                Some(wry::webview::RpcResponse::new_result(
                                    req.id.clone(),
                                    Some(Value::from(ui_client.client.lock().get_all_json())),
                                ))
                            } else {
                                Some(wry::webview::RpcResponse::new_result(
                                    req.id.clone(),
    ...
    #[allow(unused_mut)]
    fn sso_auth_window_main(&self, args: &Vec<String>) {
        let mut event_loop = wry::application::event_loop::EventLoop::new();
        let window = wry::application::window::WindowBuilder::new()
            .with_visible(false)
    ...
    fn tray_main(&self) {
        ...
        */
        ...
        let refresh = || {
            ...
            if self.http_client.client.lock().is_online() {
                let address = self
                    .http_client
                    .client
                    .lock()
                    .get_str(&["status", "address"]);
                ...
                let networks = self.http_client.client.lock().networks();
                if !networks.is_empty() {
                    ...
                    for network in networks.iter() {
                        ...
                        let assigned_addrs =
                            nw_obj.get("assignedAddresses").unwrap_or(&Value::Null);
                        ...
                        let (nwid, client2) =
                            ((*network).0.clone(), self.http_client.client.clone());
                        network_menu.push(TrayMenuItem::Text {
                        ...
                        managed_routes.as_array().map(|managed_routes| {
                            if !managed_routes.is_empty() {
                                ...
                                for r in managed_routes.iter() {
                                    r.as_object().map(|r| {
                                        let via =
                                            r.get("via").map_or("", |s| s.as_str().unwrap_or(""));
                ...
                let saved_networks = self.http_client.client.lock().saved_networks();
                ...
                #[cfg(target_os = "macos")]
                {
                    let dirty_flag2 = self.http_client.dirty_flag.clone();
                    menu.push(TrayMenuItem::Text {
                ...
                #[cfg(windows)]
                {
                    ...
                    menu.push(TrayMenuItem::Text {
                        ...
                        handler: Some(Box::new(move || {
                            ...
                            let hkcu = winreg::RegKey::predef(winreg::enums::HKEY_CURRENT_USER);
                            let startup = hkcu
                                .create_subkey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run"); // opens read/write if exists
                            if startup.is_ok() {
        ...
        let client2 = self.http_client.client.clone();
            ...
            loop {
                ...
                for network in auth_needed_networks.iter() {
                    ...
                    if auth_window.is_some() {
                        ...
                        let _ =
                            auth_windows.insert(nwid.clone(), (auth_url.clone(), Mutex::new(None))); // KEY -> (URL, WINDOW)
...
fn main() {
    ...
    let http_client = HTTPClient::new(vec!["status", "network"], 250, 10);
    ...
    if args.len() >= 2 {
        match args[1].as_str() {
            "window" => {
                ...
                if args.len() >= 3 {
                    app.control_panel_window_main(&args);
                } else {
                    println!(
